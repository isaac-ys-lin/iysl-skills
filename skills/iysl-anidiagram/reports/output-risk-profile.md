...
This skill owns animated explanatory diagrams authored as hand-written SMIL animated SVGs. `scripts/render_svg.py` validates the authoring contract and readability in a browser; PNG, MP4, and GIF are derived only when explicitly requested. The primary deliverable is the editable `.svg`.
...
- `render_svg.py --check` renders each frame in a real browser and measures text collisions and canvas margins from in-page `getBoundingClientRect` (more accurate than estimated widths) at two timeline points, then verifies the animation actually moves (`motion_nonzero`). `--png` adds a poster check without ffmpeg; `--mp4` and `--gif` add their encoded-output checks.
