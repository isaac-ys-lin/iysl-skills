...
for (const chart of chartTypes) {
  ...
  cards.push(`<section id="${chart}"><h2>${esc(name)}</h2><p class="conclusion">${esc(spec.title)}</p><p>${esc(question)} ${esc(spec.subtitle)}</p><p><a href="${chart}.svg" download>SVG</a> · <a href="${chart}.json" download>資料 JSON</a></p><div class="chart" tabindex="0" aria-label="${esc(name)}，窄螢幕可左右捲動">${svg}</div></section>`);
}
...
const nav = chartTypes.map(id => `<a href="#${id}">${esc(catalog[id].name)}</a>`).join('');
...
body{font:16px Arial,"Microsoft JhengHei","PingFang TC",sans-serif;color:#111;margin:32px auto;padding:0 24px;max-width:1200px}h1,h2{color:#000099}p{line-height:1.6}.conclusion{font-size:1.2rem;font-weight:700}nav{display:flex;gap:10px 20px;flex-wrap:wrap}a{color:#28317B}section{margin:48px 0;border-top:1px solid #ddd;padding-top:16px}.chart{overflow:auto}svg{display:block;width:100%;min-width:960px;height:auto}code{overflow-wrap:anywhere}@media(max-width:600px){body{margin:20px auto;padding:0 16px}}
</style><h1>iysl-transglobe</h1><p>${esc(provenance.scope)} 每張圖均可下載完整可重製 spec，保留結論、視覺編碼、期間與資料來源。</p><nav>${nav}</nav>${cards.join('')}<p>離線重製：下載整個圖庫目錄後執行 <code>node render-chart.js ranking.json output.svg</code>；各圖均附原始資料與產圖程式。</p></html>`);
console.log(`wrote ${cards.length} source-derived SVG charts to ${out}`);
