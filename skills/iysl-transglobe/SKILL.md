...
產圖時依格式指引選 `layout` 並填實際置入寬度，確認成品字級、比例與容器高度；
不把全頁簡報尺寸套到所有容器。周邊內容沿用編修契約；純圖可直接交付。
修改文件的任務以完成替換的文件副本為主交付，SVG／資料／生成來源為附件；
依[格式與驗收](references/formats-and-validation.md#驗收證據)分別驗證內容保真、
文件結構與實際渲染，修正檢查到的問題。
交付時簡述重大編修、可編輯範圍、已驗證的程式與平台，以及未驗證或缺資料的部分。
