...
function waterfall(s, m) {
  ...
  let out = '';
  for (const v of ticks(lo, hi)) out += line(p.x, Y(v), p.x + p.w, Y(v)) + text(p.x - 12, Y(v) + 5, tick(v), { 'text-anchor': 'end', fill: C.muted, 'font-size': 14 });
...
function funnel(s, m) {
  ...
  let out = text(p.x + p.w + 12, p.y - 18, '件數 / 階段完成率', { fill: C.muted, 'font-size': 14 });
  data.forEach((r, i) => {
    const y = p.y + i * slot + 8, width = p.w * r.value / base;
    ...
    out += rect(p.x + (p.w - width) / 2, y, width, slot - 16, i === data.length - 1 ? C.blue : C.gray, { 'data-count': r.value });
...
function tornado(s, m) {
  ...
  const [lo, hi] = extent(data.flatMap(r => [r.low, r.high, s.baseline])), p = plot(m, 240, 220), X = v => scale(v, lo, hi, p.x, p.x + p.w), slot = p.h / data.length;
  if (slot < 45) fail('tornado rows too dense; increase height or split');
  ...
  out += text(p.x + p.w + 16, p.y - 18, '下界 / 上界結果', { fill: C.muted, 'font-size': 14 });
  data.forEach((r, i) => {
  ...
  return out + text(p.x, p.y + p.h + 55, `基準：${s.baseline}；灰＝下界結果，藍＝上界結果`, { fill: C.muted, 'font-size': 14 });
}
...
function render(spec) {
  ...
  const titleLines = wrap(spec.title, (w - 88) / 28);
  ...
  const footer = [`單位：${spec.unit}　期間：${spec.period}`, `資料來源：${spec.source}`, ...spec.notes, ...extra].flatMap(v => wrap(v, (w - 88) / 14));
  const footerTop = h - 28 - (footer.length - 1) * 20;
  const m = { w, h, unit: spec.unit, plotTop: 68 + titleLines.length * 34, plotBottom: footerTop - 108 };
  ...
  return el('svg', { xmlns: 'http://www.w3.org/2000/svg', viewBox: `0 0 ${w} ${h}`, width: w, height: h, role: 'img', 'aria-labelledby': `${id}-title ${id}-desc`, 'font-family': 'Arial, Microsoft JhengHei, PingFang TC, sans-serif' },
    el('title', { id: `${id}-title` }, esc(spec.title)) + el('desc', { id: `${id}-desc` }, esc([spec.title, spec.unit, spec.period, spec.source, ...spec.notes, ...extra].join('；'))) +
    el('metadata', {}, esc(JSON.stringify(spec))) + rect(0, 0, w, h, '#FFFFFF') + titleLines.map((v, i) => text(44, 44 + i * 34, v, { fill: '#000099', 'font-size': 28, 'font-weight': 700 })).join('') + body +
    footer.map((v, i) => text(44, footerTop + i * 20, v, { fill: C.muted, 'font-size': 14 })).join(''));
}
