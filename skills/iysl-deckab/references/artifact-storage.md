...
Use this contract when `$iysl-deckab` work needs file-backed artifacts. Keep artifacts in one run folder so chat decisions, outline truth, prompts, anchors, images, and review notes do not drift apart.
...
```text
work/iysl-deckab-runs/YYYYMMDD-HHMMSS-<short-topic-slug>/
```
