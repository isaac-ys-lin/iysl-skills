...
Status: Merged locally — verified; push pending
...
- Out of scope: 不加入新的 runtime 依賴、不建立自動套版器、不處理 pipeline 成本分級；不擴充既有的 `iysl-ytdlp-html-report` 功能。
- Acceptance criteria: motion regression tests 能在舊行為失敗並在修正後通過；所有 anidiagram SVG 通過結構、循環與 render gates；平行 render 不再產生不同 frame stream 或 hang；skill/package/release verifier 在隔離 worktree 通過；目標 commits 整合到本機 `main`；沒有新增快取或背景程序殘餘。
...
- **Confirmed** — merge 前既有的 ytdlp skill 屬於 `main` 的獨立 commit；本次只處理與 anidiagram 的衝突交集，不擴充其功能。
...
- 2026-07-13：本機 `main` 已 merge `anidiagram-style-diversity`，並保留既有 ytdlp skill；README 與 package inventory 衝突以五個 skills 的聯集解決。
- 2026-07-13：merge review 排除誤入的 `.firecrawl/search-iysl-sync.json` 外部搜尋快取，並以 `.gitignore` 防止再次納入。
- Fresh detached merge worktree：package contract `8 passed`、anidiagram `59 passed`、clarify `5 passed`、sync `8 passed`、ytdlp `8 passed`，`tools/verify-release.sh` 回報 `portable release gates passed`；`tools/verify-npx-install.sh` 安裝五個 skills 並通過 parity。
- 下一步：等待使用者明確授權後推送 `main`。
