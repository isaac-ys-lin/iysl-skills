...
Use this contract when `$deck-ab` work needs file-backed artifacts. Keep artifacts in one run folder so chat decisions, outline truth, prompts, anchors, images, and review notes do not drift apart.
...
```text
work/deck-ab-runs/YYYYMMDD-HHMMSS-<short-topic-slug>/
```
