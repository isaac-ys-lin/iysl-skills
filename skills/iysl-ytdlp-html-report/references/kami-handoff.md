...
5. **表格保持表格**。`comparison` 與 `control-gap` 的欄列關係就是它的內容，改寫成
   卡片或條列會把「同一維度可比較」這件事弄丟。第一欄是標籤欄，要給得起讓中文標籤
   排成一行的寬度；被壓到逐字換行（「控制／點」）時，讀者是從一堆直排單字去猜這一列
   在講什麼。
...
- **不要替章節寫導言。** 「這一章要講什麼」那種過場句，即使完全由 spec 推導得出、
  即使寫得準，也是沒有證據回指的讀者內容。章節的開場只能用 block 的 `summary`；
  真的需要一段結構性的過場，就把它宣告成 `data-report-chrome`，承認它沒有證據。
  這一條驗證器擋不住（見下一節），所以它靠你自己守。

欄位標籤、序號、`適用條件：` 這類把 spec 欄位講清楚的前綴，屬於排版，不受這一條限制。
界線是：它有沒有替讀者多下一個判斷。
...
這不是假設性的風險。實測一次出稿，四章各被加了一句章節導言，例如「影片把數字給齊了，
但有三個判斷它留在原地」——句子準確、也只可能從 spec 推導出來，但它替讀者多下了一個
判斷，而且沒有任何 evidence 回指得到它。四個錨點、七個 block、brief 全部通過驗證。
要抓這一類，只能逐段把 HTML 的文字比對 spec 的字串；驗證器不做這件事。
