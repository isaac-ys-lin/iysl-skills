...
assert(narrowStack.includes('窄區塊：甲／B 2（2%）') && narrowStack.includes('甲／C 0（0%）'), 'small and zero stacked segments have companion labels');
...
assert(mekko.includes('甲／B 0（0%）') && mekko.includes('data-value="0"'), 'zero mekko cells remain explicit');
...
assert((indexed.match(/>Q3</g) || []).length === 2 && indexed.includes('共用非零指數尺度'), 'missing values do not remove period labels and scale is disclosed');
